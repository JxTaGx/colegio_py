...
class Menu:
    ...
    def seleccionar_entidad(self, tipo_entidad, estado_requerido=None, mensaje_extra=""):
        ...
        if not lista:
            print(f"Info - No hay {tipo_entidad}s registrados.")
            return None

        print(f"--- Seleccionar {tipo_entidad.capitalize()} {mensaje_extra}---")
        items_validos = []
        for i, item in enumerate(lista):
            if estado_requerido is None or item.get_estado() == estado_requerido:
                items_validos.append(item)
                nombre = item.get_nombre() if hasattr(item, 'get_nombre') else item.get_nombre_completo()
                print(f"{len(items_validos)}. Código: {item.get_codigo()}, Nombre: {nombre}")
        
        if not items_validos:
            criterio_str = f" (Estado: {estado_requerido})" if estado_requerido else ""
            print(f"Info - No hay {tipo_entidad}s que cumplan con el criterio{criterio_str}.")
            return None

        while True:
            try:
                opcion = int(input(f"Seleccione el número del {tipo_entidad}: "))
                if 1 <= opcion <= len(items_validos):
                    return items_validos[opcion - 1]
                else:
    ...
    def registrar_entidad(self, tipo):
        ...
        try:
            if tipo == "Estudiante":
                ...
                # sin_espacios1 = ["nombres"]
        
                # #LEN
                # if len(sin_espacios1 < 3:
                ...
                if not grado_obj: return
                params['grado'] = grado_obj
                
                # sin_espacios2 = ["apellidos"]
                
                # #LEN
                # if len(sin_espacios2) <= 5:
                #     print("El apellido es demasiado corto.")
                #     return False
                ...
                params['apellidos'] = self.solicitar_cadena("Digite los apellidos: ", permitir_numeros=False)
                materia_obj = self.seleccionar_entidad("Materia", "Habilitado")
                if not materia_obj: return
                aula_obj = self.seleccionar_entidad("Aula", "Habilitado")
                if not aula_obj: return
                params['materia'] = materia_obj
                params['aula'] = aula_obj
                
            elif tipo in ["Materia", "Grado", "Aula"]:
                print("Por favor ingresar el grado en letras (Septimo)")
                ...
                if not sin_espacios.isalpha():
                    ...
                    return False
                #LEN
                if len(sin_espacios) < 4:
                    print("El nombre es demasiado corto.")
                    return False
    
                params['nombre'] = sin_espacios
    ...
    def listar_entidades(self, tipo, titulo):
        ...
        for item in lista:
            if tipo == "Grado":
                est_en_grado = len([e for e in self.colegio.estudiantes if e.get_grado() == item and e.get_estado() == 'Habilitado'])
    ...
    def visualizar_entidad(self, tipo):
        ...
        if entidad:
            if tipo == "Grado":
                est_en_grado = len([e for e in self.colegio.estudiantes if e.get_grado() == entidad and e.get_estado() == 'Habilitado'])
    ...
    def registrar_asistencia(self):
        ...
        if aula_obj.get_estado_ocupacion() != 'Disponible':
        ...
        for est in estudiantes_del_grado:
            ...
            if asistio == 's':
                ...
                # else:
                # tiene_excusa = self.solicitar_cadena("¿Tiene excusa? (s/n): ").lower()
                # justificacion = ""
                # if tiene_excusa == "s":
    ...
    def menu_reportes(self):
        while True:  
            ...
            if opcion == '1':
                ...
                print(f"\nLa cantidad total de estudiantes habilitados es: {total}")
