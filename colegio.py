...
class Colegio:
    ...
    def adicionar_entidad(self, tipo, **kwargs):
        ...
        if tipo == "Estudiante":
            ...
            est_en_grado = len([e for e in self.estudiantes if e.get_grado() == grado_obj and e.get_estado() == 'Habilitado'])
            ...
            if len(self.aulas) >= self.max_aulas:
                return f"Error - Se ha alcanzado el número máximo de {self.max_aulas} aulas."
    ...
    def modificar_entidad(self, tipo, codigo, campo, nuevo_valor):
        ...
        if hasattr(entidad, setter_name):
            if tipo == "Estudiante" and campo == "grado":
                est_en_grado = len([e for e in self.estudiantes if e.get_grado() == nuevo_valor and e.get_estado() == 'Habilitado'])
    ...
    def validar_fecha_asistencia(self, fecha_obj):
        ...
        if fecha_obj < fecha_inicio_escolar or fecha_obj > date.today():
            return "La fecha debe estar dentro del año escolar (desde 20/01/2025 hasta hoy)."
    ...
    def registrar_asistencia(self, codigo, fecha_obj, aula_obj, docente_obj, grado_obj, presentes, ausentes):
        ...
        for est in presentes:
            for asistencia_existente in self.asistencias:
                if asistencia_existente.get_fecha() == fecha_obj and est in asistencia_existente.get_presentes():
